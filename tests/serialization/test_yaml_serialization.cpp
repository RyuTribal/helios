...
TEST(YamlSerializerTest, MeshRendererRoundTrip) {
    ...
    original.mesh_path     = "Meshes/test.hvemesh";
    original.material_path = "Materials/default.hvemat";
    original.flags         = MeshFlags::CastShadows;
    original.sort_order    = 5;
    ...
    EXPECT_EQ(result.mesh_path, "Meshes/test.hvemesh");
    EXPECT_EQ(result.material_path, "Materials/default.hvemat");
    EXPECT_EQ(result.flags, MeshFlags::CastShadows);
    EXPECT_EQ(result.sort_order, 5);
}
    ...
    // --- Save ---
    {
        ...
        world.spawn(
            ...
            Transform{glm::vec3(0, 0, 0),
                      ...
                      glm::vec3(1, 1, 1)},
            MeshRenderer{
                .mesh = Handle<MeshAsset>::from(AssetHandle{1, 1}),
                .material = Handle<MaterialAsset>::from(AssetHandle{2, 1}),
                .flags = MeshFlags::CastShadows | MeshFlags::ReceiveShadows,
                .mesh_path = "Meshes/test.hvemesh"}
        );
        ...
        world.archetypes().for_each_archetype([&](Archetype& arch) {
            ...
            for (size_t row = 0; row < arch.size(); ++row) {
                ...
                if (tag.name == "player") {
                    ...
                    // Paths persist, handles don't (session-local)
                    EXPECT_EQ(mr.mesh_path, "Meshes/test.hvemesh");
                    EXPECT_EQ(mr.flags, MeshFlags::CastShadows | MeshFlags::ReceiveShadows);
                }
            }
        });
...
TEST(SceneSerializerTest, LoadMissingFileHandledGracefully) {
    World world;
    SceneSerializer serializer;
    // YAML::LoadFile throws internally for missing files -- this is outside our code.
    // The serializer now logs + returns on parse errors, but a missing file
    // still throws from yaml-cpp itself. We just verify no crash for a valid
    // but missing-root file.
    auto tmp = std::filesystem::temp_directory_path() / "helios_empty_scene.yaml";
    {
        std::ofstream f(tmp);
        f << "not_scene:\n  foo: bar\n";
    }
    // Should not crash -- just logs and returns without adding entities.
    serializer.load(world, tmp);
