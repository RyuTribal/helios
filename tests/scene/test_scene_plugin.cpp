...
// NOTE: load_scene_from_yaml calls YAML::LoadFile which throws for missing
// files (that's yaml-cpp, not our code). We test the graceful path
// (missing 'scene' root) instead.
TEST_F(ScenePluginTest, MissingSceneRootIsGraceful) {
    SceneManagerPlugin plugin;
    SceneManager mgr;

    plugin.build_factories_for_test();

    // Write a YAML file missing the 'scene' root key
    auto bad_path = m_test_dir / "bad_root.yaml";
    {
        std::ofstream f(bad_path);
        f << "not_scene:\n  foo: bar\n";
    }

    auto h = mgr.create("bad_root");
    // Should not crash -- just logs and returns without loading entities
    load_scene_from_yaml(mgr, h, bad_path, plugin.factories_for_test());
}
