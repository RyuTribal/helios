...
TEST(DAGBuilder, ConflictingSystemsSameStageWithoutExplicitOrder) {
    // Both write CompA -> conflict but no explicit ordering.
    // Like Bevy: unordered conflicting systems run in the same stage
    // (arbitrary order). Use explicit .after/.before to sequence them.
    std::vector<SystemDescriptor> systems = {
        make_system({1}, { { std::type_index(typeid(CompA)), AccessMode::Write } }),
...
TEST(DAGBuilder, ReadWriteConflictNoImplicitOrder) {
    // System 0 reads CompA, System 1 writes CompA -> conflict but no explicit order.
    // Without explicit ordering, both run in the same stage (Bevy behavior).
    std::vector<SystemDescriptor> systems = {
        make_system({1}, { { std::type_index(typeid(CompA)), AccessMode::Read } }),
        make_system({2}, { { std::type_index(typeid(CompA)), AccessMode::Write } }),
    };
...
TEST(DAGBuilder, ConflictingSystemsWithExplicitOrder) {
    // Two systems that conflict: both write A.
    // With explicit ordering (system 2 after system 1), they are in separate stages.
    std::vector<SystemDescriptor> systems = {
        make_system({1}, { { std::type_index(typeid(CompA)), AccessMode::Write } }),
        make_system({2}, { { std::type_index(typeid(CompA)), AccessMode::Write } },
                    /*after=*/{ SystemId{1} }),
...
TEST(DAGBuilder, EventWriterWriterNoImplicitOrder) {
    // Two EventWriter<T> conflict but no explicit ordering → same stage (Bevy behavior).
    std::vector<SystemDescriptor> systems = {
        make_system({1}, { { std::type_index(typeid(MyEvent)), AccessMode::Write } }),
        make_system({2}, { { std::type_index(typeid(MyEvent)), AccessMode::Write } }),
    };
    auto plan = build_execution_plan(systems);
    ASSERT_EQ(plan.stages.size(), 1u) << "Without explicit ordering, conflicting systems share a stage";
}

TEST(DAGBuilder, EventReaderWriterNoImplicitOrder) {
    // EventReader<T> vs EventWriter<T> conflict but no explicit ordering → same stage.
    std::vector<SystemDescriptor> systems = {
