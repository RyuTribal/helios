...
namespace hve{
    ...
    HVEBuffer::~HVEBuffer() {
        unmap();
        vkDestroyBuffer(hveDevice.device(), buffer, nullptr);
        vkFreeMemory(hveDevice.device(), memory, nullptr);
    }
    ...
    VkResult HVEBuffer::map(VkDeviceSize size, VkDeviceSize offset) {
        ...
        return vkMapMemory(hveDevice.device(), memory, offset, size, 0, &mapped);
    }
    ...
    void HVEBuffer::unmap() {
        if (mapped) {
            vkUnmapMemory(hveDevice.device(), memory);
            mapped = nullptr;
        }
    }
    ...
    VkResult HVEBuffer::flush(VkDeviceSize size, VkDeviceSize offset) {
        ...
        return vkFlushMappedMemoryRanges(hveDevice.device(), 1, &mappedRange);
    }
    ...
    VkResult HVEBuffer::invalidate(VkDeviceSize size, VkDeviceSize offset) {
        ...
        mappedRange.offset = offset;
        mappedRange.size = size;
        return vkInvalidateMappedMemoryRanges(hveDevice.device(), 1, &mappedRange);
    }
     ...
     *
     */
    VkResult HVEBuffer::flushIndex(int index) { return flush(alignmentSize, index * alignmentSize); }
