...
TEST(EventStorage, UnregisteredChannelThrows) {
    EventStorage storage;
    EXPECT_THROW(storage.get_channel<DamageEvent>(), std::out_of_range);
    EXPECT_THROW(storage.writer<DamageEvent>(), std::out_of_range);
    EXPECT_THROW(storage.reader<DamageEvent>(), std::out_of_range);
}
