...
//
// Pre-creates all Forward+ graphics and compute pipelines at startup.
// Each block loads SPIR-V shaders, builds descriptor layouts, and creates
// the pipeline via PipelineCache.  Pipelines that fail to create (e.g.
// missing .spv files) are logged as warnings but do not prevent startup.
