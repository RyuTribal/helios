...
inline void serialize_yaml(YAML::Emitter& out, const MeshRenderer& mr) {
    out << YAML::BeginMap;
    out << YAML::Key << "mesh_path"     << YAML::Value << mr.mesh_path;
    out << YAML::Key << "material_path" << YAML::Value << mr.material_path;
    out << YAML::Key << "flags"         << YAML::Value << mr.flags;
    out << YAML::Key << "sort_order"    << YAML::Value << mr.sort_order;
    out << YAML::EndMap;
}
...
inline void serialize_yaml(YAML::Emitter& out, Camera::ClearMode v) {
    switch (v) {
        case Camera::ClearMode::SolidColor: out << "SolidColor"; break;
        case Camera::ClearMode::None:       out << "None";       break;
    }
}
...
inline void serialize_yaml(YAML::Emitter& out, const Camera& c) {
    ...
    out << YAML::Key << "order"           << YAML::Value << c.order;
    out << YAML::Key << "viewport_x"      << YAML::Value << c.viewport_x;
    out << YAML::Key << "viewport_y"      << YAML::Value << c.viewport_y;
    out << YAML::Key << "viewport_w"      << YAML::Value << c.viewport_w;
    out << YAML::Key << "viewport_h"      << YAML::Value << c.viewport_h;
    out << YAML::Key << "clear_mode"      << YAML::Value; serialize_yaml(out, c.clear_mode);
    out << YAML::Key << "render_schedule" << YAML::Value << c.render_schedule;
    out << YAML::EndMap;
}

inline void serialize_yaml(YAML::Emitter& out, const RenderLayers& rl) {
    out << YAML::BeginMap;
    out << YAML::Key << "mask" << YAML::Value << rl.mask;
    out << YAML::EndMap;
}
...
inline void serialize_yaml(YAML::Emitter& out, const SceneRoot& sr) {
    out << YAML::BeginMap;
    out << YAML::Key << "scene_name" << YAML::Value << sr.scene_name;
    out << YAML::Key << "scene_path" << YAML::Value << sr.scene_path;
    out << YAML::EndMap;
}
...
inline MeshRenderer deserialize_yaml_mesh_renderer(const YAML::Node& node) {
    MeshRenderer mr;
    // Path-based persistence — handles are resolved at runtime by the editor
    if (node["mesh_path"])     mr.mesh_path     = node["mesh_path"].as<std::string>("");
    if (node["material_path"]) mr.material_path = node["material_path"].as<std::string>("");
    if (node["flags"])         mr.flags         = node["flags"].as<uint32_t>();
    if (node["sort_order"])    mr.sort_order    = node["sort_order"].as<int32_t>(0);
    // Legacy support: read old handle format if path not present
    if (mr.mesh_path.empty() && node["mesh"]) {
        mr.mesh = Handle<MeshAsset>::from(deserialize_yaml_asset_handle(node["mesh"]));
    }
    return mr;
}
...
inline Camera::ClearMode deserialize_yaml_clear_mode(const YAML::Node& node) {
    auto s = node.as<std::string>();
    if (s == "None") return Camera::ClearMode::None;
    return Camera::ClearMode::SolidColor;
}
...
inline Camera deserialize_yaml_camera(const YAML::Node& node) {
    ...
    if (node["order"])           c.order            = node["order"].as<int32_t>();
    if (node["viewport_x"])      c.viewport_x       = node["viewport_x"].as<float>();
    if (node["viewport_y"])      c.viewport_y       = node["viewport_y"].as<float>();
    if (node["viewport_w"])      c.viewport_w       = node["viewport_w"].as<float>();
    if (node["viewport_h"])      c.viewport_h       = node["viewport_h"].as<float>();
    if (node["clear_mode"])      c.clear_mode       = deserialize_yaml_clear_mode(node["clear_mode"]);
    if (node["render_schedule"]) c.render_schedule  = node["render_schedule"].as<std::string>();
    return c;
}

inline RenderLayers deserialize_yaml_render_layers(const YAML::Node& node) {
    RenderLayers rl;
    if (node["mask"]) rl.mask = node["mask"].as<uint32_t>();
    return rl;
}
...
inline SceneRoot deserialize_yaml_scene_root(const YAML::Node& node) {
    SceneRoot sr;
    if (node["scene_name"]) sr.scene_name = node["scene_name"].as<std::string>();
    if (node["scene_path"]) sr.scene_path = node["scene_path"].as<std::string>();
    return sr;
}
