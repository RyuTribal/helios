...
struct Window::Impl {
    ...
    GLFWcursor*        blank_cursor = nullptr;
    WindowDesc         desc;
    ...
    ~Impl() {
        if (blank_cursor) {
            glfwDestroyCursor(blank_cursor);
            blank_cursor = nullptr;
        }
        if (glfw_window) {
...
void Window::set_cursor_mode(CursorMode mode) {
    if (mode == CursorMode::Captured) {
        // Create a 1x1 transparent cursor to visually hide it
        if (!m_impl->blank_cursor) {
            unsigned char pixels[4] = {0, 0, 0, 0};
            GLFWimage img = {1, 1, pixels};
            m_impl->blank_cursor = glfwCreateCursor(&img, 0, 0);
        }
        glfwSetCursor(m_impl->glfw_window, m_impl->blank_cursor);
        ...
        m_impl->cursor_captured = true;
    } else {
        if (m_impl->cursor_captured) {
            glfwSetInputMode(m_impl->glfw_window, GLFW_RAW_MOUSE_MOTION, GLFW_FALSE);
            m_impl->cursor_captured = false;
        }
        glfwSetInputMode(m_impl->glfw_window, GLFW_CURSOR, GLFW_CURSOR_NORMAL);
        glfwSetCursor(m_impl->glfw_window, nullptr); // restore default cursor
    }
}

void Window::warp_cursor_to_grab() {
    // No-op on Wayland (cursor position is managed by the compositor in disabled mode)
}

Window::CursorMode Window::cursor_mode() const {
    return m_impl->cursor_captured ? CursorMode::Captured : CursorMode::Normal;
}
