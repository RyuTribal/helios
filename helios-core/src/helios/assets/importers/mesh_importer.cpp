...
// Load optimized .hvemesh binary directly into a MeshAsset.
// Returns the MeshAsset via std::any, or empty on failure.
std::any load_optimized_hvemesh(const std::filesystem::path& path,
                                AssetServer& server) {
    ...
    // Read optimized mesh data -- direct memcpy, no cgltf parsing.
    // Format: [uint32 vert_count] [uint32 idx_count] [PBRVertex[]] [uint32[]]
    auto& r = result->data;
    uint32_t vert_count = r.read<uint32_t>();
    uint32_t idx_count  = r.read<uint32_t>();

    if (vert_count == 0 || idx_count == 0) {
        HELIOS_LOG(Assets, Error, "Optimized mesh binary has 0 verts/indices: {}", path.string());
        return std::any{};
    }

    MeshAsset mesh;
    mesh.vertices.resize(vert_count);
    if (!r.read_bytes(reinterpret_cast<uint8_t*>(mesh.vertices.data()),
                      vert_count * sizeof(PBRVertex))) {
        HELIOS_LOG(Assets, Error, "Truncated vertex data in: {}", path.string());
        return std::any{};
    }
    mesh.indices.resize(idx_count);
    if (!r.read_bytes(reinterpret_cast<uint8_t*>(mesh.indices.data()),
                      idx_count * sizeof(uint32_t))) {
        HELIOS_LOG(Assets, Error, "Truncated index data in: {}", path.string());
        return std::any{};
    }

    // --- Read embedded material data ---
    if (!r.at_end()) {
        float base_r = r.read<float>();
        float base_g = r.read<float>();
        float base_b = r.read<float>();
        float metallic = r.read<float>();
        float roughness = r.read<float>();

        MaterialAsset mat;
        mat.base_color = {base_r, base_g, base_b};
        mat.metallic = metallic;
        mat.roughness = roughness;

        // Read embedded textures (albedo, normal, metallic_roughness, emissive)
        std::string path_str = path.string();
        auto read_texture = [&](const std::string& name) -> Handle<TextureAsset> {
            uint32_t w = r.read<uint32_t>();
            uint32_t h = r.read<uint32_t>();
            uint32_t sz = r.read<uint32_t>();
            ...
            tex.height = h;
            tex.hdr = false;
            tex.pixels.resize(sz);
            if (!r.read_bytes(tex.pixels.data(), sz)) return {};

            auto handle = server.store<TextureAsset>(
                path_str + "#" + name, std::move(tex));
            server.acquire(handle);
            return Handle<TextureAsset>::from(handle);
        };

        mat.albedo = read_texture("albedo");
        mat.normal = read_texture("normal");
        mat.metallic_roughness = read_texture("metallic_roughness");
        mat.emissive = read_texture("emissive");

        // Store the material as a sub-asset
        auto mat_handle = server.store<MaterialAsset>(
            path_str + "#material", std::move(mat));
        server.acquire(mat_handle);
        ...
        mesh.default_material = Handle<MaterialAsset>::from(mat_handle);
    }

    return std::any(std::move(mesh));
}
...
std::any MeshImporter::import(const std::filesystem::path& path, AssetServer& server) {
    if (!std::filesystem::exists(path)) {
        HELIOS_LOG(Assets, Error, "Mesh file not found: {}", path.string());
        return std::any{};
    }

    // Optimized .hvemesh: read PBRVertex binary and convert to MeshData.
    if (path.extension() == ".hvemesh") {
        auto mesh_any = load_optimized_hvemesh(path, server);
        if (!mesh_any.has_value()) return std::any{};
        auto& mesh = std::any_cast<MeshAsset&>(mesh_any);

        MeshData mesh_data;
        mesh_data.source_path = path.string();
        mesh_data.vertices.reserve(mesh.vertices.size());
        for (const auto& pv : mesh.vertices) {
            Vertex v;
        ...
        mesh_data.indices = std::move(mesh.indices);
    ...
    cgltf_result result = cgltf_parse_file(&options, actual_path.string().c_str(), &data);
    if (result != cgltf_result_success) {
        HELIOS_LOG(Assets, Error, "cgltf failed to parse '{}'", path.string());
        return std::any{};
    }

    result = cgltf_load_buffers(&options, data, actual_path.string().c_str());
    if (result != cgltf_result_success) {
        cgltf_free(data);
        HELIOS_LOG(Assets, Error, "cgltf failed to load buffers for '{}'", path.string());
        return std::any{};
    }
    ...
    // Extract materials
    std::filesystem::path model_dir = actual_path.parent_path();
    for (cgltf_size i = 0; i < data->materials_count; ++i) {
...
std::any MeshImporter::import_mesh_asset(const std::filesystem::path& path,
    ...
    // Optimized .hvemesh: direct memcpy load, no cgltf parsing.
    if (path.extension() == ".hvemesh") {
        return load_optimized_hvemesh(path, server);
    }
