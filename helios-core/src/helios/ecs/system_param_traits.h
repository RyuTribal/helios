...
// Systems taking World& have untracked access to all components and resources.
// We report a single Exclusive descriptor so the DAG builder places these
// systems into their own serial stage, preventing races with every other system.
template <>
struct SystemParam<World> {
    static std::vector<AccessDescriptor> accesses() {
        return { AccessDescriptor{
            .type = std::type_index(typeid(World)),
            .mode = AccessMode::Exclusive,
        } };
    }
    static World& fetch(World& world, uint32_t /*last_run_tick*/) { return world; }
};
