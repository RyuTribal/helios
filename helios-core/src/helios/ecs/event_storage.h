...
/// Typed event channel. Maintains a double-buffered pair of event vectors.
///
/// Thread-safety: send() is NOT internally synchronized. Safety relies on
/// the DAG scheduler serializing systems that share write access to the same
/// event type (EventWriter<T> reports AccessMode::Write on typeid(T), so two
/// writers or a writer+reader pair produce a conflict edge and run in
/// separate stages). Do not call send() from multiple threads concurrently.
template <typename T>
...
public:
    ...
    EventChannel<T>& get_channel() {
        ...
        assert(it != m_channels.end() && "EventStorage: event channel not registered");
