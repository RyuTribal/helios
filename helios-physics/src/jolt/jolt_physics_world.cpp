...
JPH::BodyInterface& JoltPhysicsWorld::body_interface() {
    // No-lock variant: all body_interface() callers run on the main thread
    // outside step(), so there is no contention with the physics job system.
    return m_physics_system->GetBodyInterfaceNoLock();
}
...
BodyHandle JoltPhysicsWorld::create_body(const BodyDesc& desc, uint64_t entity_id) {
    ...
    if (auto* box = std::get_if<BoxShape>(&desc.shape)) {
        ...
        if (result.HasError()) {
            HELIOS_LOG(Physics, Error, "BoxShape creation failed: {}", result.GetError().c_str());
            return BodyHandle{0};
        }
        shape = result.Get();
        ...
        auto result = settings.Create();
        if (result.HasError()) {
            HELIOS_LOG(Physics, Error, "SphereShape creation failed: {}", result.GetError().c_str());
            return BodyHandle{0};
        }
        shape = result.Get();
        ...
        auto result = settings.Create();
        if (result.HasError()) {
            HELIOS_LOG(Physics, Error, "CapsuleShape creation failed: {}", result.GetError().c_str());
            return BodyHandle{0};
        }
        shape = result.Get();
    }
    ...
    if (shape == nullptr) {
        HELIOS_LOG(Physics, Error, "create_body: unknown or unsupported shape variant");
        return BodyHandle{0};
    }
    ...
    if (body == nullptr) {
        HELIOS_LOG(Physics, Error, "create_body: Jolt CreateBody failed (body limit reached?)");
        return BodyHandle{0};
    }
    ...
    // If this handle already maps to a body, destroy the old one to prevent leaks.
    auto existing = m_handle_to_body.find(handle);
    if (existing != m_handle_to_body.end()) {
        HELIOS_LOG(Physics, Warn, "create_body: handle {} already exists, destroying old body", handle);
