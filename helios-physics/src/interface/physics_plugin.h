...
#include "interface/physics_log_channel.h"
...
inline void physics_step(helios::ResMut<std::unique_ptr<PhysicsWorld>> world,
    ...
    // Guard: the editor nulls the physics world in Edit mode to pause physics.
    if (!*world) return;
...
// Destroy physics bodies when Collider or RigidBody is removed at runtime.
// Without this, a body would persist with a stale shape after component removal.
inline void physics_collider_cleanup(
    helios::ResMut<std::unique_ptr<PhysicsWorld>> world,
    helios::ResMut<PhysicsBodyMap>                body_map,
    helios::Query<const PhysicsBody,
                  helios::Without<Collider>>      missing_collider,
    helios::Query<const PhysicsBody,
                  helios::Without<helios::RigidBody>> missing_rigidbody,
    helios::Commands&                             cmds)
{
    // Guard: the editor nulls the physics world in Edit mode to pause physics.
    if (!*world) return;

    // Entities that lost their Collider component
    for (auto [entity, pb] : missing_collider.with_entity()) {
        HELIOS_LOG(Physics, Warn, "Entity lost Collider, destroying physics body (handle {})", pb.handle);
        (*world)->destroy_body(pb.handle);
        body_map->unregister_body(entity);
        cmds.remove<PhysicsBody>(entity);
    }

    // Entities that lost their RigidBody component
    for (auto [entity, pb] : missing_rigidbody.with_entity()) {
        HELIOS_LOG(Physics, Warn, "Entity lost RigidBody, destroying physics body (handle {})", pb.handle);
        (*world)->destroy_body(pb.handle);
        body_map->unregister_body(entity);
        cmds.remove<PhysicsBody>(entity);
    }
...
//
// KNOWN LIMITATION: This writes world-space positions directly into the local
// Transform component. Physics bodies must currently be root entities (no
// parent) -- parented entities will have their local transform overwritten
// with world-space values, breaking the hierarchy.
...
struct PhysicsPlugin {
    ...
    void build(auto& app) {
        ...
        // Destroy bodies when Collider or RigidBody is removed at runtime
        app.add_system(helios::Schedule::PreUpdate, physics_collider_cleanup, "physics_collider_cleanup");
