...
namespace Engine
{
	...
	{
		HVE_CORE_ERROR_TAG("LinuxWindow", "GLFW Error ({0}): {1}", error, description);
	}
	...
	void LinuxWindow::SetTitle(std::string& new_title)
	{
		glfwSetWindowTitle(m_Window, new_title.c_str());
	}

	void LinuxWindow::SetFullScreen(bool fullscreen, FullscreenType type)
	{
	...
	void LinuxWindow::SetMaximized(bool maximized)
	{
		if (maximized) {
	...
	bool LinuxWindow::IsKeyPressed(uint32_t key)
	{
		return s_KeyStates[key];
	}

	void LinuxWindow::SetKeyState(uint32_t key, bool state)
	{
		s_KeyStates[key] = state;
	}

	void LinuxWindow::ClearKeyStates()
	{
		s_KeyStates.clear();
	}
	...
	void LinuxWindow::Init(const WindowProps& props)
	{
		...
		glfwSetWindowMaximizeCallback(m_Window, LinuxWindow::OnMaximize);
	...
	void LinuxWindow::OnMaximize(GLFWwindow* window, int maximized)
	{
		WindowData& data = *(WindowData*)glfwGetWindowUserPointer(window);
		data.ScreenMaximized = maximized;
	}

	void LinuxWindow::OnSizeChange(GLFWwindow* window, int width, int height)
	{
		WindowData& data = *(WindowData*)glfwGetWindowUserPointer(window);
		data.Width = width;
		data.Height = height;
