    ...
    Res<std::shared_ptr<AssetServer>>                                                            asset_server)
{
    ...
    for (auto [t, cam, layers] : cameras) {
        cam_entries.push_back({t, cam, layers ? layers->mask : 1u});
    }
    ...
    for (const auto& entry : cam_entries) {
        ...
        cv.camera = renderer::CameraData{
            ...
            .projection   = (cam.projection == ProjectionType::Perspective)
                ? glm::perspective(
                      ...
                      cam.far_plane)
                : glm::ortho(
                      -cam.ortho_size * aspect, cam.ortho_size * aspect,
        ...
        cv.layer_mask = entry.layers;

        // Resolve render schedule label
        if (!cam.render_schedule.empty()) {
            cv.schedule_label_value = registry->find(cam.render_schedule).value;
        }
    ...
    for (auto [t, mr, layers] : meshes) {
        packet->mesh_draws.push_back(renderer::MeshDraw{
            .transform     = t.to_mat4(),
            .mesh          = mr.mesh.untyped().packed(),
            .material      = mr.material.untyped().packed(),
            .render_layers = layers ? layers->mask : 1u,
            .sort_order    = mr.sort_order,
        });
    }

    // Sort by sort_order so draw calls respect user-defined ordering.
    // Stable sort preserves insertion order within the same sort_order.
    std::stable_sort(packet->mesh_draws.begin(), packet->mesh_draws.end(),
        [](const renderer::MeshDraw& a, const renderer::MeshDraw& b) {
    ...
    if (*asset_server) {
        gpu_cache->evict_unused(**asset_server, *render_ctx->device);
    }
}
