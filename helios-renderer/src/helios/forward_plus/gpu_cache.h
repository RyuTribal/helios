...
public:
    ...
    void evict_unused(const AssetServer& server, rhi::Device& device);
    ...
    /// Get-or-upload a cubemap from HDR source (equirect->cube via GPU compute).
    rhi::Texture* get_or_upload_cubemap(AssetHandle handle,
                                         const CubeMapAsset& asset,
                                         rhi::Device& device);

    /// Flat normal default texture (128, 128, 255, 255)
    rhi::Texture* get_or_create_default_normal(rhi::Device& device);
...
private:
    ...
    std::unique_ptr<rhi::Texture> m_default_normal;

    std::unordered_map<uint64_t, std::unique_ptr<rhi::Texture>> m_cubemaps;

    // Synthetic 1x1 textures (e.g. metallic-roughness defaults), keyed by
    // parent material handle. Separate map avoids key collisions with real textures.
    std::unordered_map<uint64_t, std::unique_ptr<rhi::Texture>> m_synthetic_textures;
};
