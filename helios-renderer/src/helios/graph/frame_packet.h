...
struct MeshDraw {
    ...
    uint32_t render_layers = 1; // from RenderLayers component (default: layer 0)
    int32_t  sort_order = 0;    // from MeshRenderer::sort_order
};
...
struct CameraView {
    ...
    uint32_t schedule_label_value = 0;  // RenderScheduleLabel::value (0 = default)
    uint32_t layer_mask = 1;   // from RenderLayers component (default: layer 0)

    // Per-camera render target. null = default scene framebuffer (scene_color/scene_depth).
    rhi::Texture* target_color = nullptr;
    rhi::Texture* target_depth = nullptr;
