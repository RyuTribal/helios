...
bool VulkanSwapchain::present()
{
    ...
    present_info.pWaitSemaphores    = &m_render_finished[m_current_image_index];
...
void VulkanSwapchain::create_swapchain(const SwapchainDesc& desc)
{
    ...
    // Log the actual present mode (vk-bootstrap may fall back from the desired mode)
    const char* mode_names[] = {"Immediate", "Mailbox", "Fifo", "FifoRelaxed"};
    int mode_idx = static_cast<int>(vkb_swapchain.present_mode);
    HELIOS_LOG_INFO(Renderer, "Swapchain actual present mode: {} ({})",
        (mode_idx >= 0 && mode_idx <= 3) ? mode_names[mode_idx] : "Unknown", mode_idx);
...
void VulkanSwapchain::create_sync_objects()
{
    ...
    // Per-frame-slot: image_available semaphores + in-flight fences
    for (uint32_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    ...
    // Indexed by acquired image index so a semaphore is only reused after its
    // image is re-acquired (which implies the prior present completed).
    uint32_t image_count = static_cast<uint32_t>(m_images.size());
    m_render_finished.resize(image_count, VK_NULL_HANDLE);
    for (uint32_t i = 0; i < image_count; i++) {
        vkCreateSemaphore(device, &sem_info, nullptr, &m_render_finished[i]);

        std::string name = "RenderFinishedSem_img" + std::to_string(i);
        m_device->context().set_debug_name(device, VK_OBJECT_TYPE_SEMAPHORE,
            reinterpret_cast<uint64_t>(m_render_finished[i]), name.c_str());
    }
}
...
void VulkanSwapchain::blit_from(rhi::CommandBuffer& cmd, rhi::Texture& src,
                                uint32_t src_width, uint32_t src_height)
{
    ...
    VkCommandBuffer raw = vk_cmd.vk_command_buffer();
    VkImage sc_image = m_images[m_current_image_index];

    // src: ShaderReadOnly -> TransferSrc
    VulkanTexture::transition_layout(raw, vk_src.vk_image(),
        VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
        VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL);

    // swapchain: Undefined -> TransferDst
    VulkanTexture::transition_layout(raw, sc_image,
    ...
    // src: TransferSrc -> ShaderReadOnly (restore for subsequent frames)
    VulkanTexture::transition_layout(raw, vk_src.vk_image(),
        VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
        VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
    vk_src.set_current_layout(VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
...
void VulkanSwapchain::destroy()
{
    ...
    // Destroy per-image render_finished semaphores
