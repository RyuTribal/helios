...
public:
    ...
    void transition_image(rhi::Texture& texture,
                          TextureLayout old_layout,
                          TextureLayout new_layout) override;

    // Dynamic rendering (no render pass / framebuffer)
    void begin_rendering(rhi::Texture* color, rhi::Texture* depth,
                         const ClearValues& clear,
                         uint32_t width, uint32_t height) override;
    void end_rendering() override;

    // Clear
    void clear_image(rhi::Texture& texture, float r, float g, float b, float a) override;

    // Transfer
    void copy_buffer(const rhi::Buffer& src, const rhi::Buffer& dst, uint32_t size) override;
    void blit_image(const rhi::Texture& src, const rhi::Texture& dst,
                    uint32_t src_width, uint32_t src_height,
                    uint32_t dst_width, uint32_t dst_height) override;
