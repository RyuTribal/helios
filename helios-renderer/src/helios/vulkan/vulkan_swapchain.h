...
public:
    ...
    void blit_from(rhi::CommandBuffer& cmd, rhi::Texture& src,
...
private:
    ...
    // image_available + fences: indexed by frame-in-flight slot (protected by fence wait)
    VkSemaphore m_image_available[MAX_FRAMES_IN_FLIGHT]{};
    VkFence m_in_flight_fences[MAX_FRAMES_IN_FLIGHT]{};

    // render_finished: indexed by swapchain image index (protected by image re-acquisition)
    // This avoids the semaphore reuse hazard when image_count > MAX_FRAMES_IN_FLIGHT
