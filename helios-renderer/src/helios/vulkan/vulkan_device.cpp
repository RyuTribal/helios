...
void VulkanDevice::destroy()
{
    if (m_device != VK_NULL_HANDLE) {
        ...
        // Flush all deferred deletion queues so resources are destroyed
    ...
    const rhi::DescriptorSetLayout& layout)
{
    ...
    for (int attempt = 0; attempt < 2; ++attempt) {
        VkDescriptorPool pool = m_descriptor_pools.back();
        alloc_info.descriptorPool = pool;
        VkResult result = vkAllocateDescriptorSets(m_device, &alloc_info, &set);
        if (result == VK_SUCCESS) {
            return std::make_unique<VulkanDescriptorSet>(*this, set, pool);
        }
