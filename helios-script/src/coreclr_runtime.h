...
///
/// Use the static create() factory; returns nullptr on failure (logged, no exceptions).
class CoreCLRRuntime final : public ScriptRuntime {
public:
    /// Factory: create and fully initialize the CoreCLR runtime.
    /// Returns nullptr on failure (logged, no exceptions).
    static std::unique_ptr<CoreCLRRuntime> create(
